...
#### list = regex.findall(string, pos, endpos, flags=0)
...
- 参数
  ...
  - flags 功能标志位
- 返回值
  - 返回匹配到的内容列表，如果正则中有子组，则列表中元素为子组对应内容

### Match_Object对象属性和方法

- 属性
  - obj.pos 匹配目标字符串开始位置
  - obj.endpos 匹配目标字符串结束位置
  - obj.re 正则表达式
  - obj.string 目标字符串
  - obj.lastgroup 最后一组的组名
  - obj.lastindex 最后一组的序号
- 方法
  - start() 获取匹配内容的开始位置
  - end() 获取匹配内容的结束位置
  - span() 获取匹配内容的起止位置
  ...
  - groups() 获取子组对应内容
  - group() 获取match对象对应内容，默认得到全部匹配内容，如果指定组序号或组名，则得到对应组内容
